...
قد تحتاج مكوناتك في كثير من الأحيان إلى عرض أشياء مختلفة اعتمادًا على حالات مختلفة. في React، يمكنك عرض JSX شَرطيّا باستعمال صيغة JavaScript مثل تعليمات `if`، `&&` و عمليات `? :`. 
...
يمكنك كتابة ذلك باستخدام [تعليمة `if`/`else`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/if...else) على النحو التالي:
...
من الناحية العملية، إعادة قيمة `null` من مكون ليست شائعة لأنها قد تفاجئ المطور الذي يحاول عرضها. في كثير من الأحيان، ستقوم بتضمين المكون شَرطيّا أو استبعاده في JSX الخاص  بالمكون الأب. إليك كيفية القيام بذلك!
...
بينما هذا التكرار ليس ضارًا، إلا أنه قد يجعل الكود أكثر صعوبة في الصيانة. ماذا لو أردت تغيير قيمة className؟ ستضطر للقيام بذلك في مكانين في الكود! في مثل هذه الحالة ، يمكنك تضمين القليل من JSX شَرطيّا لجعل الكود الخاص بك أكثر [DRY.](https://en.wikipedia.org/wiki/Don%27t_repeat_yourself)

### العامل (الثلاثي) الشرطي (`? :`) {/*conditional-ternary-operator--*/}

يحتوي JavaScript على صيغة مضغوطة لكتابة تعبير شرطي -- [العامل الشرطي](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Conditional_Operator) أو ”العامل الثلاثي“.
...
#### هل هذين المثالين متساويين تمامًا؟ {/*are-these-two-examples-fully-equivalent*/}

إذا كنت قادمًا من خلفية البرمجة كائنية التوجه ، فقد تفترض أن المثالين أعلاه يختلفان اختلافًا طفيفًا لأن أحدهما قد ينشئ "مثالين" مختلفين من `<li>`. لكن عناصر JSX ليست "أمثلة" لأنها لا تحتوي على أي حالة داخلية وليست عُقد DOM حقيقية. إنما هي أوصاف خفيفة، مثل المخططات. إذن هذان المثالان ، في الواقع ، متساويان تمامًا. يتم شرح كيفية [الحفاظ على الحالة وإعادة تعيينها](/learn/preserving-and-resetting-state) بالتفصيل.

</DeepDive>

الآن لنفترض أنك ترغب في وضع النص الخاص بالعناصر المكتملة في عنصر HTML إضافي ك`<del>` لشطبه. يمكنك إضافة المزيد من الأسطر الجديدة والأقواس حتى يكون من الأسهل تضمين JSX إضافي في كل حالة:
...
هذا النمط يعمل بشكل جيد للشروط البسيطة، ولكن استخدمه باعتدال. إذا أصبحت مكوناتك فوضوية بسبب العلامات الشرطية المتداخلة، ففكر في استخراج المكونات الفرعية لتنظيف الأمور. في React، يعد الترميز جزءًا من الكود الخاص بك، لذا يمكنك استخدام أدوات مثل المتغيرات والدوال لترتيب التعبيرات المعقدة.

### العامل المنطقي AND (`&&`) {/*logical-and-operator-*/}

الاختصار الشائع الآخر الذي ستصادفه هو [العامل المنطقي AND (&&) في JavaScript](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/Logical_AND#:~:text=The%20logical%20AND%20(%20%26%26%20)%20operator,it%20returns%20a%20Boolean%20value.). داخل مكونات React، غالبًا ما تظهر عندما تريد عرض بعض JSX فقط عندما يكون الشرط صحيحًا، **وعدم عرض أي شيء في حالة الشرط الغير صحيح**. باستخدام العامل `&&`، يمكنك عرض علامة التحقق شَرطيّا فقط إذا كانت قيمة `isPacked` صحيحة `true`:
...
يمكنك قرائتها على النحو التالي *"إذا `isPacked`، إذا (`&&`) اعرض علامة التحقق، وإلا، لا تقم بعرض أي شيء"*
...
### إسناد JSX شرطيًا إلى متغير {/*conditionally-assigning-jsx-to-a-variable*/}
...
استخدم تعليمة `if` لإعادة تعيين تعبير JSX إلى `itemContent` إذا كانت قيمة `isPacked` صحيحة `true`.
...
[الأقواس المنحنية تفتح ”نافذة إلى JavaScript“.](/learn/javascript-in-jsx-with-curly-braces#using-curly-braces-a-window-into-the-javascript-world) قم بتضمين المتغير بواسطة الأقواس المنحنية في شجرة JSX المُرجَعة، مدمجًا التعبير المحسوب سابقا داخل JSX:
...
* يمكنك إرجاع تعبير JSX شرطيًا باستخدام تعليمة `if`.
...
* الاختصارات شائعة، ولكن ليس عليك استخدامها إذا كنت تفضل استخدام `if`.
...
#### إعادة هيكلة سلسلة من عبارات `? :` إلى تعليمة `if` ومتغيرات. {/*refactor-a-series-of---to-if-and-variables*/}
