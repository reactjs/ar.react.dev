...
تخيل أن مكوناتك طهاة في المطبخ، يصنعون أطباقًا لذيذة من الوصفات، في هذا السيناريو، React هو النادل الذي يسجل طلبات الزبائن وتقديمها لهم. هذه العملية من طلب وتقديم الواجهة لها ثلاث خطوات:
...
2. تم تحديث حالة المكون (أو أحد أسلافه). (state)

### التصيير الأوَّلِي {/*initial-render*/}

عند بدء تشغيل التطبيق، تحتاج لبدء التصيير الأوَّلِي، تخفي إطارات العمل ومحررات الأكواد البسيطة هذا الكود، لكنه يتم بواسطة استدعاء [`createRoot`](/reference/react-dom/client/createRoot) مستعدفًا عنصر DOM، ثم استدعاء طريقة `render` مع المكون الخاص بك:
...
* **عند التصيير الأوَّلِي،** سينشئ React المكون الأصل (الجذر).
...
في المثال التالي، س React `Gallery()` و `Image()` عدة مرات:
