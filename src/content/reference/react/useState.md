...
على الرغم من أن نتيجة `createInitialTodos()`تستخدم فقط للترجمة الأولية، إلا أنك لا تزال تدعو هذه الدالة في كل ترجمة. هذا يمكن أن يكون تبذير إذا كان يخلق مصفوفة كبيرة أو إجراء حسابات مكلفة.
...
* **إذا كان بإمكانك حساب القيمة التي تحتاجها بالكامل من الخصائص الحالية أو حالة أخرى, [فاحذف هذه الحالة المتكررة بشكل كامل.](/learn/choosing-the-state-structure#avoid-redundant-state)** إذا كنت قلقًا من إعادة الحساب بشكل متكرر، فال [`useMemo` خطاف](/reference/react/useMemo) يمكنه المساعدة.
