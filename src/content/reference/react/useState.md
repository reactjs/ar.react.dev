...
<Intro>
 ...
 `useState` هو خطاف في React يتيح لك إضافة [متغير حالة](/learn/state-a-components-memory) إلى مكونك
...
* إذا مررت دالة كـ `initialState،` سيتم التعامل معها كقيمة أوّلية. يجب أن تكون نقية، ويجب ألا تأخذ أي معاملات، ويجب أن تعيد قيمة من أي نوع. سيستدعي React هذه الدالة عند تقديم المكون، ويستخدم القيمة التي ترجع منها كحالة أولية. [انظر المثال بالأسفل.](#avoiding-recreating-the-initial-state)
...
* `useState`هو خطاف، لذلك يمكنك استدعاؤها فقط **في المستوى الأعلى من مكوناتك** أو الخطاطيف المخصصة. لا يمكنك استدعاؤها داخل الحلقات (loop)أو الشروط. إذا كنت بحاجة إلى ذلك، استخرج مكون جديد وانقل الحالة فيه.
* في الوضع الصارم، يستدعي React **الدالة الأولية مرتين** من أجل [مساعدتك في العثور على الأخطاء البرمجية.] هذا السلوك في بيئة التطوير فقط ولا يؤثر على التطبيق التام في مرحلة الإنتاج. إذا كانت وظيفتك المبدئية نقية (كما ينبغي أن تكون)، فلا ينبغي أن يؤثر ذلك على السلوك. سيتم تجاهل أحد الاستدعاءين.

---

### الدالة `set` مثل `setSomething(nextState)` {/*setstate*/}

تسمح لك الدالة `set` التي ترجعها `useState` بتحديث الحالة إلى قيمة مختلفة وتشغيل إعادة التقديم. يمكنك تمرير الحالة التالية مباشرة، أو دالة تحسب الحالة التالية من الحالة السابقة
```js
...
* `nextState`:القيمة التي تريد أن تكون عليها الحالة. يمكن أن تكون قيمة من أي نوع، ولكن هناك سلوك خاص للدوال.
  * إذا مررت دالة كـ `nextState`، سيتم التعامل معها كدالة _تحديث_. يجب أن تكون نقية، يجب أن تأخذ الحالة الحالية كمعامل وحيد، ويجب أن تعيد الحالة التالية. React سيضع دالة التحديث في طابور ويعيد تقديم المكون الخاص بك. خلال العرض التالي، يقوم React بحساب الحالة التالية من خلال تطبيق كل التحديثات الموجودة في قائمة الانتظار على الحالة السابقة. [انظر مثالاً أدناه.](#updating-state-based-on-the-previous-state)

#### المرجعات {/*setstate-returns*/}

لا ترجع الدالة `set` أي شيء.

#### ملاحظات {/*setstate-caveats*/}

* الدالة `set` **تقوم بتحديث متغير الحالة فقط للتصيير التالي**. إذا قرأت متغير الحالة بعد استدعاء الدالة `set`, [ستظل تحصل على القيمة القديمة](#ive-updated-the-state-but-logging-gives-me-the-old-value) التي كانت على الشاشة قبل استدعائك.

* إذا كانت القيمة الجديدة التي تقدمها مطابقة للحالة الحالية، كما تحددها مقارنة ['Object.is'](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Object/is)، فإن React سيتخطى **عملية إعادة التصيير للمكون وأبناءه**. هذا هو التحسين. على الرغم من أن React في بعض الحالات لا يزال بحاجة إلى استدعاء المكون الخاص بك قبل تخطي الأطفال، فإنه لا ينبغي أن يؤثر على كودك.

* React [batches state upbatches state updatesdates.](/learn/queueing-a-series-of-state-updates) يقوم بتحديث الشاشة بعد أن يقوم جميع معالجي الأحداث بتشغيل *** وتسمية وظائف "المجموعة" الخاصة بهم. وهذا يمنع إعادة العروض المتعددة خلال حدث واحد. في الحالات النادرة التي تحتاج فيها إلى تحديث الشاشة في وقت سابق، على سبيل المثال للوصول إلى DOM، يمكنك استخدام [`flushSync`.](/reference/react-dom/flushSync)
