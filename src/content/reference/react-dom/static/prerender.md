---
title: "prerender (التصيير المسبق)"
---

<Intro>

تُصيِّر الدالة `prerender` شجرة React إلى HTML ساكن باستخدام [تدفق ويب (Web Stream)](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API).
...
تعتمد هذه الواجهة على [تدفقات الويب (Web Streams).](https://developer.mozilla.org/en-US/docs/Web/API/Streams_API) في Node.js استخدم [`prerenderToNodeStream`](/reference/react-dom/static/prerenderToNodeStream) بدلًا منها.
...
استدعِ `prerender` لتصيير تطبيقك إلى HTML ساكن.
...
* **اختياري** `options`: كائن يحوي خيارات التوليد الساكن.
  ...
  * **اختياري** `onError`: دالة استدعاء تُنفَّذ عند حدوث خطأ على الخادم، سواء كان [قابلاً للاسترداد](/reference/react-dom/server/renderToReadableStream#recovering-from-errors-outside-the-shell) أو [غير قابل للاسترداد.](/reference/react-dom/server/renderToReadableStream#recovering-from-errors-inside-the-shell) افتراضيًا تستدعي `console.error` فقط. إن استبدلتها [لتسجيل تقارير الأعطال،](/reference/react-dom/server/renderToReadableStream#logging-crashes-on-the-server) فتأكد أنك ما زلت تستدعي `console.error`. يمكنك أيضًا استخدامها [لضبط رمز الحالة](/reference/react-dom/server/renderToReadableStream#setting-the-status-code) قبل إصدار الغلاف (shell).
  * **اختياري** `progressiveChunkSize`: عدد البايتات في كل مقطع (chunk). [اقرأ المزيد عن الافتراضي.](https://github.com/facebook/react/blob/14c2be8dac2d5482fda8a0906a31d239df8551fc/packages/react-server/src/ReactFizzServer.js#L210-L225)
...
تُرجِع `prerender` وعدًا (Promise):
- إذا نجح التصيير، يُحقَّق الوعد بكائن يحتوي على:
  ...
  - `postponed`: كائن معتم يمكن تسلسله بصيغة JSON ويُمرَّر إلى [`resume`](/reference/react-dom/server/resume) إذا لم تنتهِ `prerender`. وإلا `null` يدل على أن `prelude` يحتوي كل المحتوى ولا حاجة لاستدعاء `resume`.
...
### متى أستخدم `prerender`؟ {/*when-to-use-prerender*/}

تُستخدم الواجهة الساكنة `prerender` لتوليد HTML من جانب الخادم بشكل ساكن (SSG). على عكس `renderToString`، تنتظر `prerender` تحميل كل البيانات قبل أن تُحقِّق الوعد. لذلك تناسب توليد HTML ساكن لصفحة كاملة، بما في ذلك البيانات التي تُجلب عبر Suspense. لبث المحتوى أثناء تحمّله، استخدم واجهة SSR تدعم البث مثل [`renderToReadableStream`](/reference/react-dom/server/renderToReadableStream).

يمكن إيقاف `prerender` ثم متابعته لاحقًا إما بـ `resumeAndPrerender` أو بـ `resume` لدعم التصيير المسبق الجزئي.
...
استدعِ `prerender` لتصيير شجرة React إلى HTML ساكن في صورة [Readable Web Stream:](https://developer.mozilla.org/en-US/docs/Web/API/ReadableStream)
...
```js {1-5,8,9}
// ستحتاج للحصول على هذا JSON من أداة البناء، مثلًا بقراءته من مخرجات البناء.
const assetMap = {
...
async function handler(request) {
  const {prelude} = await prerender(<App assetMap={assetMap} />, {
    // انتبه: من الآمن استدعاء stringify() هنا لأن هذه البيانات ليست من المستخدم.
...
استدعِ `prerender` لتصيير تطبيقك إلى سلسلة HTML ساكنة:
...
تنتظر `prerender` تحميل كل البيانات قبل إنهاء توليد HTML الساكن وتحقيق الوعد. مثلًا صفحة ملف شخصي تعرض غلافًا، شريطًا جانبيًا فيه الأصدقاء والصور، وقائمة منشورات:
...
تخيّل أن `<Posts />` يحتاج لتحميل بيانات تستغرق وقتًا. مثاليًا تريد انتظار انتهاء المنشورات لتُضمَّن في HTML. لذلك يمكنك استخدام Suspense للتعليق على البيانات، وستنتظر `prerender` انتهاء المحتوى المعلّق قبل أن تُحقِّق إلى HTML الساكن.
...
يمكن استخدام ذلك للتصيير المسبق الجزئي مع [`resume`](/reference/react-dom/server/resume) أو [`resumeAndPrerender`](/reference/react-dom/static/resumeAndPrerender).
...
استجابة `prerender` تنتظر انتهاء تصيير التطبيق بالكامل، بما في ذلك انتظار حل كل حدود Suspense، قبل أن تُحقِّق. صُممت لتوليد المواقع ساكنًا (SSG) مسبقًا ولا تدعم بث المزيد من المحتوى أثناء تحمّله.

لبث المحتوى أثناء تحمّله، استخدم واجهة SSR تدعم البث مثل [`renderToReadableStream`](/reference/react-dom/server/renderToReadableStream).
